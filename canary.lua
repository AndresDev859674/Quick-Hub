...
local Window = Rayfield:CreateWindow({
   ...
   KeySettings = {
      ...
      Note = "How to Get the Password, Go to README.md on github page and find the Password",
      FileName = "Key",
      SaveKey = true,
...
local Paragraph = TabHelp:CreateParagraph({Title = "The Compatibility Is", Content = "Xeno, JJSploit (not tested), And Alls Exploits, UNC RECOMMENDED 75% or 84%"})
...
local Button = TabPlayersLog:CreateButton({
    ...
    Callback = function()
        ...
        local notification = game:GetService("StarterGui"):SetCore("SendNotification", {
            Title = "Confirmation",
            Text = "Do you want to enable player logging?",
            Duration = 5, -- Duration in seconds
            Button1 = "Yes", -- Button 1
            Button2 = "No"   -- Button 2
        })

        -- Connect the confirmation function
        notification.Button1Down:Connect(function()
            -- If the user presses "Yes", execute the code
            print("User confirmed. Enabling player logging...")

            -- Table to track displayed players
            ...
            local function onPlayerJoined(player)
                if displayedPlayers[player] then return end -- Avoid duplicates
                displayedPlayers[player] = true

                local username = player.Name
                addLogMessage("Player Joined: " .. username)

                -- Connect player-specific events
                player.Chatted:Connect(function(message)
                    addLogMessage("Player Chatted: " .. username .. " - " .. message)
                end)

                -- Detect when the player dies or respawns
                player.CharacterAdded:Connect(function(character)
                    -- Check if HumanoidRootPart is present
                    ...
                    -- Event to detect when the player dies
                    humanoid.Died:Connect(function()
                        addLogMessage("This Player is Dead!: " .. username)
                    end)

                    -- Event to detect when the player respawns
            ...
            local function onPlayerLeft(player)
                if not displayedPlayers[player] then return end -- Avoid duplicates
                displayedPlayers[player] = nil

                local username = player.Name
                addLogMessage("Player Left: " .. username)
            end
            ...
            -- Show players already in the game when the UI is opened
        ...
        notification.Button2Down:Connect(function()
            print("User declined. Player logging will not be enabled.")
        end)
    end,
})
